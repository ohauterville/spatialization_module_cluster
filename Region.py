...
class Region:
    ...
    def get_osm_by_tag(
        self, place_name: str, tag: str, place_geometry=None, crs="EPSG:4326"
    ):
        """
        ...
        if place_geometry:
            if crs != "EPSG:4326":
                gdf = gpd.GeoDataFrame(
                    index=[0], crs=crs, geometry=[place_geometry]
                )  # Assuming the original CRS is WGS84 (EPSG:4326)
    ...
    def add_gis(self, file, name: str, year: str, lvl: int):
        ...
        elif file.endswith(".shp") or file.endswith(".gpkg"):
    ...
    def merge_output_dfs(self, col: str):
        ...
        self.output_df_merged = reduce(
            lambda left, right: pd.merge(left, right, on=col), df_list
        )
            ...
            if row[parent_region_col] == self.name:
                ...
                for gis in self.gis_list:
                    if (
                        not os.path.isfile(
                            os.path.join(
                                os.path.dirname(gis.file),
                                os.path.join(self.name, "subregions"),
            ...
            if row[parent_region_col] == self.name:  # OECD specific
                try:
                    ...
                    for output_csv_path in output_csv_paths:
                        subregion.output_df_list.append(
                            Df(
                                pd.read_csv(
                                    os.path.join(
                                        output_csv_path,
                                        subregion.parent_name,
                                        subregion.name,
                                        "_".join(years),
                                    )
                                    + ".csv"
    ...
    def make_subregions_ucdb_visual(
        self, ucdb_file, uc_col: str, parent_col: str, years
    ):
        for _, row in ucdb_file.iterrows():
            if row[parent_col] == self.name:
                subregion = Region(row[uc_col], self.lvl + 1)
                ...
                try:
                    subregion.output_df_list.append(
                        Df(
                            pd.read_csv(
                                "/data/mineralogie/hautervo/data/Outputs/UCDB/"
                                + subregion.parent_name
                                + "/"
                                + subregion.name
                                + "/"
        ...
        if type == "GHSL_OECD":
            self.output_df = pd.DataFrame(
                {
                    "year": years,
                    "GDP per capita": None,
                    "Population_OECD": None,
                    "Population_GHSL": None,
                    "Built up surface GHSL/Population_OECD": None,
                    "Population_OECD/Total surface": None,
                }
            )
            ...
            for y in years:
                ...
                for subregion in self.subregions:
                    # intensive
                    population_GHSL += subregion.output_df.loc[
                        subregion.output_df["year"] == int(y), "Population_GHSL"
                    ].sum()
                    population_OECD += subregion.output_df.loc[
                        subregion.output_df["year"] == int(y), "Population_OECD"
                    ].sum()
                    ghsl_surface += subregion.output_df.loc[
                        subregion.output_df["year"] == int(y), "Built up surface GHSL"
                    ].sum()
                    total_surface += subregion.output_df.loc[
                        subregion.output_df["year"] == int(y), "Total surface"
                    ].sum()
                    # extensive
                    gdp_sum += (
                        subregion.output_df.loc[
                            subregion.output_df["year"] == int(y), "GDP per capita"
                        ].sum()
                        * subregion.output_df.loc[
                            subregion.output_df["year"] == int(y), "Population_OECD"
                        ].sum()
                ...
                if population_OECD != 0:
                    if gdp_sum != 0:
                        self.output_df.loc[
                            self.output_df["year"] == y, "GDP per capita"
                        ] = (gdp_sum / population_OECD)
                    else:
                        pass

                    self.output_df.loc[
                        self.output_df["year"] == y,
                        "Built up surface GHSL/Population_OECD",
                    ] = (
                        ghsl_surface / population_OECD
                    )
                    self.output_df.loc[
                        self.output_df["year"] == y, "Population_OECD"
                    ] = population_OECD
                    self.output_df.loc[
                        self.output_df["year"] == y, "Population_OECD/Total surface"
                    ] = (population_OECD / total_surface)

                # same but with the pop computed by GHSL
                if gdp_sum != 0:
                    self.output_df.loc[
                        self.output_df["year"] == y, "GDP per capita"
                    ] = (gdp_sum / population_GHSL)
                else:
                    pass

                self.output_df.loc[
                    self.output_df["year"] == y, "Built up surface GHSL/Population_GHSL"
                ] = (ghsl_surface / population_GHSL)
                self.output_df.loc[self.output_df["year"] == y, "Population_GHSL"] = (
                    population_GHSL
                )
                self.output_df.loc[
                    self.output_df["year"] == y, "Population_GHSL/Total surface"
                ] = (population_GHSL / total_surface)

                # these should always be found
                self.output_df.loc[
                    self.output_df["year"] == y, "Built up surface GHSL"
                ] = ghsl_surface
                self.output_df.loc[self.output_df["year"] == y, "Total surface"] = (
                    total_surface
                )
    ...
    def delete_lists(self, pass_on=False):
        ...
        if pass_on and self.has_subregions():
            [subregion.delete_lists(pass_on=pass_on) for subregion in self.subregions]
